...
static void update_ui(void)
{
  update_time_display();
  update_progress();
  zui_text_set_content(ZUI_TEXT(app.status_text), get_status_text());
...
static void update_responsive_layout(int width, int height)
{
  ...
  zui_text_set_size(ZUI_TEXT(app.status_text), label_size);
...
{
  app.main_panel = (ZuiWidget*)zui_panel_create();
  ...
  app.progress = (ZuiWidget*)zui_circularprogress_create();
  ...
  app.status_text = (ZuiWidget*)zui_text_create("Click to start");
  zui_text_set_size(ZUI_TEXT(app.status_text), 16);
  zui_text_set_color(ZUI_TEXT(app.status_text), ZUI_COLOR_HEX(0x888899));
  zui_panel_add_child(ZUI_PANEL(app.main_panel), app.status_text);
